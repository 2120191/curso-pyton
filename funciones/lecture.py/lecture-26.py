...
print(nueva_lista)
# tento una lista de alumnos que todos ellos aprobaron y psaron al tercer semestre
#todosestan con el seghyundo semestre por que tedremos que crear una solucion que cambie el cvampo de semestre de 2 a3
...
def objeto(e):
    ...
    return [
        e
nueva_lista=list(map(lambda el+el ("carrera")))
    ]
...
lista=[4,7,3,4,5,6,6,7,9,8,]
nueva_lista=list(filter(lambda x:x%2==0,lista))
print(nueva_lista)


lista=[0,50]
nueva_lista=list(filter(lambda x:x < ==50,lista))
print(nueva_lista)

lista_filtrada=list(filter(lambda x=x["edad"]<50,lista_amigos))